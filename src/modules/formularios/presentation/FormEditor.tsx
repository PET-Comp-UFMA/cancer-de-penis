...
export default function FormEditor({ formId, initialForm }: { user: AuthUser; formId?: string; initialForm?: FormDetail }) {
  ...
  return <Box sx={{ bgcolor: BACKGROUND, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
    ...
    <Box component="main" data-testid="editor" sx={{ flex: 1, maxWidth: 1315, mx: "auto", px: { xs: 2, md: 3 }, py: { xs: 4, sm: 5 }, width: "100%" }}>
      ...
      <Step complete={stepsComplete[2]} number="03" title="Faixas de Resultados">
        <Typography sx={{ color: TEXT, fontSize: 12, mb: 2 }}>As faixas usam percentual da pontuação (0 a 100%), não a pontuação bruta — o sistema normaliza automaticamente a soma das respostas para essa escala.</Typography>
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, justifyContent: "flex-end", mb: 2 }}>
          <Button disabled={readOnly} onClick={() => updateDefinition({ resultBands: standardResultBands() })} sx={{ border: `1px solid ${GREEN}`, color: GREEN, textTransform: "none" }}>Preencher faixas padrão (Baixo/Médio/Alto)</Button>
          <Button disabled={readOnly} onClick={() => updateDefinition({ resultBands: [...definition.resultBands, { id: id("band"), minScore: null, maxScore: null, risk: "", description: "" }] })} startIcon={<AddRoundedIcon />} sx={{ bgcolor: GREEN, color: "#fff", textTransform: "none" }}>Adicionar faixa</Button>
        </Box>
        <Stack spacing={1.5}>{definition.resultBands.map((band, index) => <Box data-testid="result-band" key={band.id} sx={{ alignItems: "center", border: `1px solid ${BORDER}`, display: "grid", gap: 1.5, gridTemplateColumns: { xs: "1fr", sm: "0.7fr 0.7fr 1fr 1.5fr auto" }, p: 2 }}><TextField disabled={readOnly} inputProps={{ min: 0, max: 100 }} label="De (%)" onChange={(event) => updateDefinition({ resultBands: definition.resultBands.map((item) => item.id === band.id ? { ...item, minScore: numberOrNull(event.target.value) } : item) })} size="small" type="number" value={band.minScore ?? ""} /><TextField disabled={readOnly} inputProps={{ min: 0, max: 100 }} label="Até (%)" onChange={(event) => updateDefinition({ resultBands: definition.resultBands.map((item) => item.id === band.id ? { ...item, maxScore: numberOrNull(event.target.value) } : item) })} size="small" type="number" value={band.maxScore ?? ""} /><Input disabled={readOnly} label="Resultado/Risco" onChange={(value) => updateDefinition({ resultBands: definition.resultBands.map((item) => item.id === band.id ? { ...item, risk: value } : item) })} value={band.risk} /><Input disabled={readOnly} label="Descrição (opcional)" multiline onChange={(value) => updateDefinition({ resultBands: definition.resultBands.map((item) => item.id === band.id ? { ...item, description: value } : item) })} value={band.description} /><IconButton aria-label={`Excluir faixa ${index + 1}`} disabled={readOnly} onClick={() => updateDefinition({ resultBands: definition.resultBands.filter((item) => item.id !== band.id) })}><DeleteOutlineRoundedIcon /></IconButton></Box>)}</Stack>
      </Step>
