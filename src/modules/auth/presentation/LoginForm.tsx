...
export default function LoginForm() {
  ...
  return <Box component="form" onSubmit={submit} noValidate>
    ...
    <TextField inputRef={usernameRef} fullWidth label="Nome de usuário" value={username} onChange={(e) => setUsername(e.target.value)} autoComplete="username" autoFocus disabled={loading} margin="normal" inputProps={{ maxLength: 64, "aria-describedby": error ? "login-error" : undefined }} />
    <TextField inputRef={passwordRef} fullWidth label="Senha" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" disabled={loading} margin="normal" inputProps={{ maxLength: 128, "aria-describedby": error ? "login-error" : undefined }} InputProps={{ endAdornment: <InputAdornment position="end"><IconButton sx={{ minWidth: 44, minHeight: 44 }} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} aria-pressed={showPassword} onClick={() => setShowPassword((value) => !value)} edge="end"><span aria-hidden="true">{showPassword ? <VisibilityOff /> : <Visibility />}</span></IconButton></InputAdornment> }} />
